...
df = DataFrame(eP =  Any[], mu_per_hour = Float64[], optmsr_response = Any[], attempts=Int64[],
               ...
               lm_inner = Float64[], diffusion_rate=Float64[], eP_surf = Float64[], 
               ...
               UT = Float64[], CE = Float64[], PE = Float64[], ME = Float64[], porin = Float64[],
...
stop_log =   log10(0.000000001)
start_log =  log10(0.00001)
range_eP_env = 10 .^ range(start_log, stop = stop_log, length = 80) # M, external substrate concentration
...
for i in 1:length(range_eP_env) 
    ...
    while term_status != MOI.LOCALLY_SOLVED && attempts < 5
        ...
        value(lm_inner), value(diff_rate), value(eP_surf),  
