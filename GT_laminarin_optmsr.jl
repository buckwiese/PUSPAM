...
include("GT_global_cell_params.jl")
include("GT_laminarin_params.jl")

kinetics = "GT"
replicate = sub_replicate
now = Dates.now()
datetime_str = Dates.format(now, "yyyymmdd_HH")
file_name = "Laminarin_$(kinetics)_rep$(replicate)_$(datetime_str).csv"


df = DataFrame(eP =  Any[], eP_gL = Float64[], mu_per_hour = Float64[], div_per_day = Float64[], optmsr_response = Any[], attempts=Int64[],
            eP_diff = Float64[], eP_degr = Float64[], eO_diff = Float64[], eO_prod = Float64[],
            density_CYTO= Float64[], density_PERI = Float64[], r_cyt = Float64[], r_pp = Float64[], vol_cy = Float64[], vol_pp = Float64[], vol_exrp=Float64[],
            lm_inner = Float64[], eP_surf = Float64[], 
...
stop_log =   log10(0.00000000001)
start_log =  log10(0.001)
range_eP_env = 10 .^ range(start_log, stop = stop_log, length = 89) # M, external substrate concentration
...
for i in 1:length(range_eP_env) 
    ...
    while term_status != MOI.LOCALLY_SOLVED && attempts < 3
        include("GT_laminarin_constr.jl")
        set_value(eP_env, range_eP_env[i]) # M, external substrate concentration
        attempts += 1  # Increment the attempts counter
        # set up optimizer
        JuMP.optimize!(model)

        # Set the callback function
        println("with ext. substrate concentration ", [value(eP_env)], " got growth rate ", [exp.(JuMP.value.(logmu))], 
        " and phis of ", [value(phi_C), value(phi_P), value(phi_M), value(phi_UT)])
        term_status = JuMP.termination_status(model)

        # Save results
        optimal_values_temp =[exp.(JuMP.value.(logmu))*60*60, 
        exp.(JuMP.value.(logmu))*60*60/log(2)*24, term_status, attempts,
        value(eP_diff), value(eP_degr), value(eO_diff), value(eO_prod),
        value(D_cy), value(D_pp), value(r_cy), value(r_pp), value(volume_cy), value(volume_pp), value(volume_excelrp),
        value(lm_inner), value(eP_surf), 
	...
	solve_priority(s) = s == MOI.LOCALLY_SOLVED ? 1 : 0
	if solve_priority(term_status) > solve_priority(best_status) ||
		(solve_priority(term_status) == solve_priority(best_status) && optimal_values_temp[1] >= optimal_values[1])
                optimal_values = optimal_values_temp
		best_status = term_status
        end
    end
    push!(df, vcat(value(eP_env), value(eP_env)*eP_mw, optimal_values[:]))
end
