...
@variables(model, begin
    ...
    eP_surf_rel >= 0, (start = 0.5) # M, substrate polysaccharide at cell surface
    eO_rel >= 0, (start = 0.1)   # number, extracellular oligosaccharide 
    pO_rel >= 0, (start = 0.1)   # number, periplasmic oligosaccharide 
    pdbO_rel >= 0, (start = 0.1) # number, periplasmic debranched oligosaccharide

    S_e_rel >= 0, (start = 0.1)  # number, periplasmic sugar
    S_c_rel >= 0, (start = 0.1) # number, intracellular substrate
    C_c_rel >= 0, (start = 0.1) # number, cellular carbon 
    P_c_rel >= 0, (start = 0.1)  # number, cellular protein 
...
@NLexpressions(model, begin
    ...
    D_pp, GH16_M*GH16_mw + TBDT_M*TBDT_mw + GH30_M*GH30_mw + GH17_M*GH17_mw + pO_M*pO_mw + pdbO_M*pdbO_mw  # g/L, cell density, assuming c makes up 30% of cell mass
    # diffusion processes
...
@constraints(model, begin
    ...
    phi_C + phi_P + phi_M + phi_UT + phi_GH16 + phi_TBDT + phi_GH30 + phi_GH17 == 0.5 # proteome allocation constraint
end)
...
@NLconstraints(model, begin
    ...
    eP_surf_rel <= 1.0      # maximum substrate polysaccharide concentration at cell surface
    eO_rel <= 1.0           # maximum extracellular oligosaccharide concentration
    pO_rel <= 1.0           # maximum periplasmic oligosaccharide concentration
    pdbO_rel <= 1.0         # maximum periplasmic debranched oligosaccharide concentration
    S_e_rel <= 1.0          # maximum periplasmic sugar concentration
    S_c_rel <= 1.0          # maximum intracellular substrate concentration
    C_c_rel <= 1.0          # maximum cellular carbon concentration
    P_c_rel <= 1.0          # maximum cellular protein concentration
